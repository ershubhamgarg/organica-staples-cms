    ...
    <div className="animate-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        <div>
          ...
          <p className="page-subtitle">Manage customer orders and track fulfillment.</p>
        </div>
      </div>
                  ...
                  orders.map((order) => (
                    <tr key={order.id} style={{ borderBottom: '1px solid var(--border-color)', transition: 'background 0.2s' }}>
                      ...
                      <td style={{ padding: '16px' }}>
                        <div>
                          <div style={{ fontWeight: 500 }}>{order.delivery_address?.name || 'Guest'}</div>
                          <div style={{ fontSize: '0.8rem', color: 'var(--text-secondary)' }}>{order.delivery_address?.email}</div>
                        </div>
                      </td>
